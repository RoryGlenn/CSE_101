...
void moveFront(List L)// If L is non-empty, sets cursor under the front element, otherwise does nothing.
{
    ...
    {
        printf("List Error: calling moveFront() to a NULL List reference\n");
    }
}
...
void moveBack(List L) // If L is non-empty, sets cursor under the back element, otherwise does nothing.
{
    ...
    {
        printf("List Error: calling moveBack() to a NULL list reference\n");
        exit(1);
    }
}
...
{
    /*
    if(L->cursor != NULL && L->currentIndex != L->length - 1)
    {
...
void insertAfter(List L, int data)// Insert new element before cursor. Pre: length()>0, index()>=0
{
    //Node N = newNode(data);
    if(L==NULL)
    {
...
//        /// is not pointing to the head of the list
//        else
//        {
//            L->head = L->head->next;
