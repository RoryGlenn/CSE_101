...
//    if(A==NULL || B==NULL)
//    {
//        printf("List Error: calling equals() on NULL List reference\size");
//        exit(1);
//    }
...
// aka insert()
void prepend(List L, long data)// Insert new element into L. If L is non-empty, insertion takes place before front element.
{
    if(L == NULL)
    {
...
void append(List L, long data)// Insert new element into L. If L is non-empty, insertion takes place after back element.
{
    if(L == NULL)
    {
...
void insertBefore(List L, long data)// Insert new element before cursor. Pre: length()>0, GetIndex()>=0
{
...
void insertAfter(List L, long data)// Insert new element before cursor. Pre: length()>0, GetIndex()>=0
{
    //Node N = newNode(data);
    if(L==NULL)
    {
